...
def send_invoice_email(to_email, project_name):
    try:
        yclag = yagmail.SMTP(user=st.secrets["email"]["from"], password=st.secrets["email"]["password"])
...
def show_dashboard():
    ...
    for i, p in enumerate(filtered_projects):
        ...
        with st.expander(f"{p.get('name', 'Unnamed')} – Template: {p.get('template', 'N/A')}"):
            ...
            render_level_checkboxes("view", pid, int(current_level), p.get("timestamps", {}), levels, editable=False)
            ...
            if not st.session_state.confirm_delete.get(confirm_key):
                ...
                if col_yes.button("✅ Yes", key=f"yes_{pid}"):
                    ...
                    st.session_state.pop(f"last_email_sent_{pid}", None)
...
def show_create_form():
    ...
    if st.session_state.selected_template:
        levels = TEMPLATES[st.session_state.selected_template].copy()
        for req in ["Invoice", "Payment"]:
            if req in levels:
                levels.remove(req)
        st.session_state.custom_levels = levels + ["Invoice", "Payment"]
    else:
        st.subheader("Customize Progress Levels")
        if not st.session_state.custom_levels:
            st.session_state.custom_levels = ["Initial"]
        for req in ["Invoice", "Payment"]:
            if req in st.session_state.custom_levels:
                st.session_state.custom_levels.remove(req)
        ...
        if st.button("➕ Add Level"):
            editable_levels.append(f"New Level {len(editable_levels) + 1}")
        st.session_state.custom_levels = editable_levels + ["Invoice", "Payment"]
    ...
    if st.button("✅ Create Project"):
        if due <= start:
            st.error("Due date must be later than start date.")
